...
func (f *Fallback) GetStatus() any {
	...
	vaultStatuses := []map[string]any{}
	for _, vault := range f.vaults {
		vaultStatus := map[string]any{
...
func (f *Fallback) StoreSecret(path string, data map[string]any) error {
	if f.current == nil {
...
func (f *Fallback) GetSecret(path string) (map[string]any, error) {
	if f.current == nil {
